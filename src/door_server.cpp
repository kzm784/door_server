...
void DoorServer::doorServerTimerCallback()
{
    if (infra_cv_img_ptr_ && depth_cv_img_ptr_)
    {
        if (!img_prev_.empty())
        {
            ...
            cv::imshow("Difference Image", diff_img);

            // Labeling
            cv::Mat labeling_img = cv::Mat::zeros(infra_cv_img_ptr_->image.size(), CV_8UC1);
            labelingHoughTransform(diff_img, labeling_img, 3.0, 0.5, 5);
            int nlabel = cv::connectedComponents(labeling_img, labeling_img, 8);

            // k-means clustering
            cv::Mat clustering_points;
            kMeansHoughTransform(diff_img, 3.0, 0.5, clustering_points);
            kMeansClustering(clustering_points, nlabel-1);

            // Judge Door state and door frame distance
            updateDoorFrameData(5.0, 2, verifical_color_img);
            caluculateCenterFrameDistance(door_frame_data_updated_, door_pos_, verifical_color_img, depth_cv_img_ptr_->image);
...
void DoorServer::kMeansHoughTransform(
    const cv::Mat& src_img,
    const double vertical_angle,
    const double strict_vertical_angle,
    cv::Mat& clustering_points)
{
      std::vector<cv::Vec2f> lines;
      cv::HoughLines(src_img, lines, 1, CV_PI / 180, 50);
    ...
    cv::Mat& depth_image)
{
    ...
    doorAvePosCalc(depth_image, door_pos);
